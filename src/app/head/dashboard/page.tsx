      ...
      try {
        ...
        if (res.ok) {
          ...
          setStats({
            ...
            approved: requests.filter(r => r.status === 'approved' || r.status === 'checked-in' || r.status === 'checked-out').length,
            rejected: requests.filter(r => r.status === 'rejected').length,
            totalVisitsToday: requests.filter(r => new Date(r.requestedDateTime).toDateString() === today).length
          });
        }
      } catch (e) {
        console.error(e);
    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-neutral-500 dark:text-neutral-400">
              Overview of your department's visit requests and activities.
            </p>
          </div>
          <Link href="/head/approvals">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Total Approved</CardTitle>
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-neutral-500 dark:text-neutral-400">Visits Today</CardTitle>
              <Users className="h-4 w-4 text-blue-500" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">{stats.totalVisitsToday}</div>
            </CardContent>
          </Card>
        </div>
      </div>
